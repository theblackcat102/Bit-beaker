 ...
 *
 * <p>This class provides a convenient frontend for working with SQL. Instead of composing
 * statements manually using string concatenation, method calls are used to construct the statement
 * one clause at a time. These methods can be chained together.
 *
...
public class SelectionBuilder {
	 ...
	 *
	 * <p>In the most basic usage, simply provide a selection in SQL {@code WHERE} statement
	 * format.
	 *
	 * <p>Example:
	 *
	 ...
	 * Instead, use positional parameters in your selection statement, then pass the user input in
	 * via the selectionArgs parameter. This prevents SQL escape characters in user input from
	 ...
	 * @param selectionArgs Values to substitute for positional parameters ('?' characters in {@code
	 ...
	 *
	 * <p>This method may only be called once. If multiple tables are required, concatenate them in
	 * SQL-format (typically comma-separated).
	 *
	 ...
	 *
	 * <p>In cases where a column is mapped to another data source (either another table, or an SQL
	 * sub-query), the column name will be replaced with a more specific, SQL-compatible
	 * representation.
	 *
